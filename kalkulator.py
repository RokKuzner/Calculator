...
def button_add():
    global f_number, math
    first_nuber = e.get()
    math = 'addition'
    f_number = float(first_nuber)
    e.delete(0, END)
...
def button_equal():
    ...
    e.delete(0, END)
    if math == 'addition':
        e.insert(0, str(float(f_number) + float(second_number)))
...
button_subtract = Button(win, text='-', padx=41, pady=20, command=button_subtract)
button_multiply = Button(win, text='*', padx=40, pady=20, command=button_multiply)
button_divide = Button(win, text='/', padx=43, pady=20, command=button_divide)
